...
ParameterEditorController::ParameterEditorController(void)
    : _parameterMgr(ecsApp()->toolbox()->parameterManager())
{
    _buildLists();
...
QStringList ParameterEditorController::searchParameters(const QString& searchText, bool searchInName, bool searchInDescriptions)
{
    ...
    for(const QString &paramName: _parameterMgr->parameterNames(_vehicle->defaultCompid())) {
        if (searchText.isEmpty()) {
            list += paramName;
        } else {
            Fact* fact = _parameterMgr->getParameter(_vehicle->defaultCompid(), paramName);
...
void ParameterEditorController::saveToFile(const QString& filename)
{
    if (!filename.isEmpty()) {
        ...
        if (!QFileInfo(filename).fileName().contains(".")) {
            parameterFilename += QString(".%1").arg("param");
        }

        QFile file(parameterFilename);

        if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            qDebug() << tr("Unable to create file: %1").arg(parameterFilename);
            return;
        }
...
bool ParameterEditorController::buildDiffFromFile(const QString& filename)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << tr("Unable to open file: %1").arg(filename);
        return false;
    }
...
void ParameterEditorController::_searchTextChanged(void)
{
    ...
    if (rgSearchStrings.isEmpty() && !_showModifiedOnly) {
        ...
        for (const QString &paraName: _parameterMgr->parameterNames(_vehicle->defaultCompid())) {
            Fact* fact = _parameterMgr->getParameter(_vehicle->defaultCompid(), paraName);
